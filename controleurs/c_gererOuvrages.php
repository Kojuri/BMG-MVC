...
switch ($action) {
    ...
    case 'supprimerOuvrage' : {
        ...
        if (isset($_GET["id"])) {
            ...
            if (!$hasErrors) {
                // rechercher des ouvrages de ce ouvrage
                $strSQL = "SELECT COUNT(*)  "
                    ."FROM ouvrage_ouvrage "
                    ."WHERE id_ouvrage = ?";
                try {
                    $ouvragesOuvrage = getValue($cnx, $strSQL, array($intID));
                    if ($ouvragesOuvrage == 0) {
                        // c'est bon, on peut le supprimer
                        $strSQL = "DELETE FROM ouvrage WHERE id_ouvrage = ?";
                        try {
                        ...
                        $tabErreurs["Erreur"] = "Cet ouvrage est référencé par des ouvrages, suppression impossible !";
                        $tabErreurs["ID"] = $intID;
                        $tabErreurs["Nom"] = $strNom;
                        $tabErreurs["Prénom"] = $strPrenom;
                        $tabErreurs["Ouvrages"] = $ouvragesOuvrage;
                        $hasErrors = true;
                    }
                }
                catch (PDOException $e) {
